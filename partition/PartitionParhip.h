...
public:
	...
	virtual void partition(int* partition, const PartitionGraph<Topo>& graph, const PartitionTarget& target, int seed = 1)
	{
		...
		idxtype edgecut;
		idxtype nparts = target.vertex_count();
		std::vector<idxtype> part(cell_count);
		ParHIPPartitionKWay(vtxdist.data(), xadj.data(), adjncy.data(), vwgt.empty() ? nullptr : vwgt.data(), adjwgt.empty() ? nullptr : adjwgt.data(), &nparts, true, seed, mode, imbalance, &edgecut, part.data(), graph.comm());
